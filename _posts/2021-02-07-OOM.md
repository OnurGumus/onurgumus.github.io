...
above. There is also a seperate discussion for volatile keyword being redundant on x86 and x64 due to [strong memory model]() these CPU's offer. But that's not true and volatile do make a difference to some applications on .NET even if the program runs under those CPU's and we will briefly see why.
...
1-) A download operation from the volatile variable will the preceding download operation to finish.
2-) An upload operation to the volatile variable will wait the preceding download operation to finish.
3-) An upload operation  from the volatile variable will wait the preceding upload operation to finish

But volatile does not forbid an upload operation for the volatile wait for the preceeding download operation completed.
